...
import saveProfile from "../../../../src/redux/apis/saveProfile";
...
describe("testing loginSaga", () => {
  ...
  it("should dispatch success action", async () => {
    const generator = jest.spyOn(saveProfile, "saveprofile").mockImplementation(() => Promise.resolve(response));
    const dispatched = []
  ...
  it("should dispatch error action", async () => {
    const generator = jest.spyOn(saveprofile, "saveprofile").mockImplementation(() => Promise.reject());
    const dispatched = []
    const result = await runSaga(
      {
        dispatch: (action) => dispatched.push(action)
